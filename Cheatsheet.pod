...
Las variables depuración de ambiente son excluidos porque son para uso
...
Tamaño de pedazo usado para operaciones I/O, en bytes. Un chunk de tamaño más 
...
Archivo configuración usado por L<Mojolicious::Plugin::Config> y
...
Implementación alternativa de L<Mojo::IOWatcher> para intentar.
...
Nivel de logueo para el aplicación L<Mojolicious>, siempre debe contener un log
...
Máximo tamaño de linea para lineas de inicio y encabezados de un mensaje HTTP
en bytes, es por defecto en  C<10240>.
...
en archivos temporales, es por defecto en C<262144>.

  MOJO_MAX_MEMORY_SIZE=2048

=head2 C<MOJO_MAX_MESSAGE_SIZE>

Máximo tamaño para mensajes de HTTP, en bytes, es por defecto en C<5242880>.
...
Máximo numero de redirecciones que L<Mojo::UserAgent> seguirá, es por defecto en C<0>.

=head2 C<MOJO_MAX_WEBSOCKET_SIZE>

Máximo tamaño para mensajes de WebSocket, en bytes, es por defecto en C<262144>.
...
C<X-Forwarded-Host> y C<X-Forwarded-HTTPS> en headers.

  MOJO_REVERSE_PROXY=1

=head2 C<MOJO_STATIC_CLASS>

La clase que el despachador de archivos estaticos siempre debe usar para encontrar C<DATA>
templates, es por defecto en C<main>.

  MOJO_STATIC_CLASS=MyApp

=head2 C<MOJO_TEMPLATE_CLASS>

La clase que el renderizador de L<Mojolicious> siempre debe usar para encontrar C<DATA> "templates",
es por defecto en C<main>.

  MOJO_TEMPLATE_CLASS=MyApp

=head2 C<MOJO_TMPDIR>

Directorio para archivos temporales como archivos grandes, por defecto usando 
